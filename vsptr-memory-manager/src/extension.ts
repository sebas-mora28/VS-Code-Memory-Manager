...
import {RemoteMemory} from './RemoteMemory'
...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
	 	vscode.commands.registerCommand('extension.vsptrMemoryManager', () => {
		...
		if (vscode.window.registerWebviewPanelSerializer) {
			...
			vscode.window.registerWebviewPanelSerializer(HeapVisualizer.viewType, {
				async deserializeWebviewPanel(webviewPanel: vscode.WebviewPanel, state: any) {
					...
					HeapVisualizer.revive(webviewPanel, context.extensionPath);
				}
			});
